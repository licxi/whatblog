		...
		<div class="row" id="page-wrapper"
						...
						<tbody>
							...
							<c:forEach items="${users }" var="user">
								<tr class="gradeU" id="user${user.id }">
									<td><a href="<c:url value='/show/blog/${user.userName}'/>" target="_blank">${user.userName }</a></td>
									<td><a href="<c:url value='/show/blog/${user.userName}'/>" target="_blank">${user.nickname }</a></td>
									...
									<td><div class="dropdown">
											...
											<ul class="dropdown-menu" role="menu"
												...
												<li role="presentation"><a role="menuitem"
													...
													onclick="modifyIsLock(${user.id },'${user.userName}')">
														<c:if test="${user.isLock == 0 }"> 锁定
															</c:if> <c:if test="${user.isLock == 1 }">
																 启用
															</c:if>
												</a></li>
			...
			<c:if test="${pages.pages > 1 }">
				<nav style="text-align: center;">
					<ul class="pagination">
						<li><a href="/whatblog/admin/userManage?page=1">首页</a></li>
						<c:forEach var="page" varStatus="status" begin="1"
							end="${pages.pages }">
							<li
								<c:if test="${pages.pageNum == status.index }">class="active"</c:if>><a
								href="/whatblog/admin/userManage?page=${status.index }">${status.index }</a></li>
						</c:forEach>
						<li><a
							href="/whatblog/admin/userManage?page=${pages.lastPage }">尾页</a></li>
